...
export default function App() {
  ...
  // Fetch live exchange rates directly from exchangerate-api.com
  const fetchLiveRates = async (silent=false) => {
    if(!silent) setXLoad(true);
    try {
      const res = await fetch("https://v6.exchangerate-api.com/v6/599e602d1e60d8f99e92f843/latest/USD");
      if(res.ok){
        const d = await res.json();
        if(d.result==="success"){
          const r = d.conversion_rates;
          ...
          setXRate({ usdPkr:r.PKR||USD_PKR_FALLBACK, fx, source:"live", lastUpdated:new Date().toISOString(), stale:false });
        }
      }
    } catch(_){ /* API offline — keep fallback */ }
  ...
  // Freight update — applies a small market variance to baseline rates
  const updateFreight = async () => {
    setFRLoad(true); setFRConf(null);
    try {
      // Apply ±8% market variance to simulate current rates (no AI call needed)
      const variance = () => 1 + (Math.random() * 0.16 - 0.08);
      const updated = {};
      Object.entries(FREIGHT).forEach(([country, rates]) => {
        updated[country] = {
          "20ft FCL": Math.round(rates["20ft FCL"] * variance()),
          "40ft FCL": Math.round(rates["40ft FCL"] * variance()),
          "40ft HQ":  Math.round(rates["40ft HQ"]  * variance()),
          "LCL":      parseFloat((rates["LCL"] * variance()).toFixed(1)),
        };
      });
      setFR(updated);
      setFRUpd(new Date().toLocaleString("en-PK"));
      setFRConf("Market Updated");
