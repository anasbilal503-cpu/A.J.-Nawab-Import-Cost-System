...
/* ═══ CONSTANTS ═══════════════════════════════════════════ */
const USD_PKR = 278.5;
const FX = { USD:1, EUR:1.08, GBP:1.27, CNY:0.138, AED:0.272, SGD:0.74, JPY:0.0067 };
...
function calcItem(item, shipment, freightShareUSD, portSharePKR) {
  const rate    = FX[item.currency]||1;
  ...
  const prodPKR=prodUSD*USD_PKR, origPKR=origUSD*USD_PKR, frPKR=addFr*USD_PKR, insPKR=insUSD*USD_PKR;
...
Adjust values based on current 2026 market conditions. No text before or after JSON.`
  ...
  const handleCalculate = () => {
    ...
    const itemRes  = items.map(item=>{
      ...
      return {...calcItem(item,g,sh,portSh),name:item.name||"Item",id:item.id,unit:item.unit,
          ...
          <header style={{padding:"12px 22px",background:T.surf,borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,gap:"12px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"16px",flexShrink:0}}>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:"9px",color:T.muted,textTransform:"uppercase"}}>USD/PKR</div>
                <div style={{fontSize:"16px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:T.accent,lineHeight:1.2}}>{USD_PKR}</div>
              </div>
